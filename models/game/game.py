...
def move_by_golden_card(game_board, actual_player, move):
    ...
    coast_of_card = vard_want_to_take.getCoasts()
    player_coins = actual_player.get_pocket()
    neaded_resources = {}
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        neaded_resources[i] = coast_of_card.takeCoinByType(i).getCount()
        # tu powinienem od neededResources odjąć ilość kart danego typu które posiadam
        if (len(actual_player.get_cards_by_type(i)) + player_coins.takeCoinByType(i).getCount())\
    ...
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        ...
        if neaded_resources[i] > 0:
            player_coins.takeCoinByType(i).changeCount((-1) * neaded_resources[i])
            game_board.get_coins().takeCoinByType(i).changeCount(neaded_resources[i])
    actual_player.get_cards_by_type('gold').remove(vard_want_to_take)
    actual_player.get_pocket().takeCoinByType('gold').changeCount(-1)
    game_board.get_coins().takeCoinByType('gold').changeCount(1)
...
def move_by_golden_card_check(actual_player, moves_list):
    ...
    card_coast = card_want_to_take.getCoasts()
    player_coasts = actual_player.get_pocket()
    neaded_resources = {}
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        neaded_resources[i] = card_coast.takeCoinByType(i).getCount()
        # tu powinienem od neaded_resources odjąć ilość kart danego typu które posiadam
        if (len(actual_player.get_cards_by_type(i)) + player_coasts.takeCoinByType(i).getCount())\
                - neaded_resources[i] < 0:
            #print("nie możesz kupić karty")
            return -1
    return 1
def move_reservation_card(game_board, actual_player, move):
    if actual_player.get_pocket().coinsCount()+1 > 15:
        return -1
    if game_board.get_coins().takeCoinByType('gold').getCount() == 0:
        return -1
...
def move_take_lord(game_board, actual_player, move):
    ...
    lord_coasts = lord_card.getCoasts()
...
def move_take_two_coins(bord_pocket, player_pocket, moves):
    if bord_pocket.takeCoinByType(moves[0]['name']).getCount() < 4:
        return -1
    if player_pocket.coinsCount()+len(moves) > 15:
        return -1
    bord_pocket.takeCoinByType(moves[0]['name']).changeCount(-2)
    player_pocket.takeCoinByType(moves[0]['name']).changeCount(2)
    return 1
def move_take_two_coins_check(board_pocket, player_pocket, moves):
    if board_pocket.takeCoinByType(moves[0]['name']).getCount() < 4:
        return -1
    if player_pocket.coinsCount()+len(moves) > 15:
        return -1
    return 1
def move_take_coin(board_pocket, player_pocket, moves):
    if player_pocket.coinsCount()+len(moves) > 15:
        return -1
    for i in moves:
        for j in board_pocket.getAllCoins():
            if i['name'] == j.getType():
                if j.getCount() == 0:
                    return -1
                else:
    ...
    for i in moves:
        for j in board_pocket.getAllCoins():
            if i['name'] == j.getType():
                player_pocket.takeCoinByType(i['name']).changeCount(1)
                j.changeCount(-1)
                break
    return 1
...
def move_take_card(game_board, actual_player, move):
    ...
    card_coasts = card_want_to_take.getCoasts()
    player_coins = actual_player.get_pocket()
    needed_resources = {}
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        needed_resources[i] = card_coasts.takeCoinByType(i).getCount()
        #tu powinienem od neededResources odjąć ilość kart danego typu które posiadam
        if (len(actual_player.get_cards_by_type(i)) + player_coins.takeCoinByType(i).getCount())\
    ...
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        ...
        if needed_resources[i] > 0:
            player_coins.takeCoinByType(i).changeCount((-1)*needed_resources[i])
            game_board.get_coins().takeCoinByType(i).changeCount(needed_resources[i])
...
def move_take_card_check(game_board, actual_player, move):
    card_want_to_take = game_board.get_card(move[0]['y'], move[0]['x'] - 1)
    card_want_to_take_coast = card_want_to_take.getCoasts()
    player_coins = actual_player.get_pocket()
    needed_resources = {}
    for i in ['diamond', 'emerald', 'sapphire', 'onyx', 'ruby']:
        needed_resources[i] = card_want_to_take_coast.takeCoinByType(i).getCount()
        #tu powinienem od neededResources odjąć ilość kart danego typu które posiadam
        if (len(actual_player.get_cards_by_type(i)) + player_coins.takeCoinByType(i).getCount())\
                - needed_resources[i] < 0:
            #print("nie możesz kupić karty")
