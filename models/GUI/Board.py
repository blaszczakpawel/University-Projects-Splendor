...
class Board():
    def __init__(self,root,board):
        ...
        for i in range(4):
            ...
            self.__lords.append(tk.Button(self.__cardBoardFrame.getFrame(), image=lord.getImage(), command=functools.partial(self.__acceptionBoard.move, f"Lord z pozycji {i} ",{'type':"lord","number":i})))
            self.__lords[len(self.__lords)-1].grid(column=i, row=0,padx=BORDER_LORDS_X, pady=BORDER_LORDS_Y)
        ...
        for i in [['high',1],['medium',2],['low',3]]:
            ...
            for j in range(4):
                ...
                self.__crystalCards[i[0]].append(tk.Button(self.__cardBoardFrame.getFrame(),image=card.getImage(), command=functools.partial(self.__acceptionBoard.move,f"Karta z pozycji (x,y)=({j+1} , {i[1]})",{'type':"card","x":j+1,'y':i[1]})))
                self.__crystalCards[i[0]][j].grid(column=j,row=i[1],padx=BORDER_CRYSTAL_X, pady=BORDER_CRYSTAL_Y)
        ...
        for i in [[0,'diamond'],[1,'ruby'],[2,'onyx'],[3,'sapphire'],[4,'emerald'],[5,'gold']]:
            ...
            button=tk.Button(self.__cardCoinsFrame.getFrame(),image=self.__board.getCoinByType(i[1]).getImage(),command=functools.partial(self.__acceptionBoard.move,f"Moneta o nazwie {i[1]}",{'type':"coin","name":i[1]}))
